#REQUIRED ARGUMENTS equipment.head entity_hurt_player head 

#CHECK IF SWAP
execute if items entity @s armor.head *[minecraft:custom_data~{entity_hurt_player:{head:{only_command:{}}}}] run return run function ci:command with entity @s equipment.head.components.minecraft:custom_data.entity_hurt_player.head

tag @s remove ci.cd
data remove storage ci:temp data
data modify storage ci:temp data set from entity @s equipment.head.components.minecraft:custom_data.entity_hurt_player.head

execute if items entity @s armor.head *[minecraft:custom_data~{entity_hurt_player:{head:{cooldown:{}}}}] run tag @s add ci.cd
execute if items entity @s armor.head *[minecraft:custom_data~{entity_hurt_player:{head:{swap_items_on_trigger:{}}}}] run scoreboard players set %swap_item ci.math 1
execute store result score %consume ci.math if items entity @s armor.head *[minecraft:custom_data~{entity_hurt_player:{head:{consume:{}}}}]
...
item modify entity @s armor.head {function:"minecraft:copy_custom_data",source:{type:"minecraft:storage",source:"ci:item"},ops:[{source:"cooldown",target:"entity_hurt_player.head.cooldown",op:"merge"}]}
