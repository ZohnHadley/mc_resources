#REQUIRED ARGUMENTS equipment.offhand entity_hurt_player offhand 

#CHECK IF SWAP
execute if items entity @s weapon.offhand *[minecraft:custom_data~{entity_hurt_player:{offhand:{only_command:{}}}}] run return run function ci:command with entity @s equipment.offhand.components.minecraft:custom_data.entity_hurt_player.offhand

tag @s remove ci.cd
data remove storage ci:temp data
data modify storage ci:temp data set from entity @s equipment.offhand.components.minecraft:custom_data.entity_hurt_player.offhand

execute if items entity @s weapon.offhand *[minecraft:custom_data~{entity_hurt_player:{offhand:{cooldown:{}}}}] run tag @s add ci.cd
execute if items entity @s weapon.offhand *[minecraft:custom_data~{entity_hurt_player:{offhand:{swap_items_on_trigger:{}}}}] run scoreboard players set %swap_item ci.math 1
execute store result score %consume ci.math if items entity @s weapon.offhand *[minecraft:custom_data~{entity_hurt_player:{offhand:{consume:{}}}}]
...
item modify entity @s weapon.offhand {function:"minecraft:copy_custom_data",source:{type:"minecraft:storage",source:"ci:item"},ops:[{source:"cooldown",target:"entity_hurt_player.offhand.cooldown",op:"merge"}]}
