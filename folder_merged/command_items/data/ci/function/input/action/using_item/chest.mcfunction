#REQUIRED ARGUMENTS equipment.chest using_item chest 

#CHECK IF SWAP
execute if items entity @s armor.chest *[minecraft:custom_data~{using_item:{chest:{only_command:{}}}}] run return run function ci:command with entity @s equipment.chest.components.minecraft:custom_data.using_item.chest

tag @s remove ci.cd
data remove storage ci:temp data
data modify storage ci:temp data set from entity @s equipment.chest.components.minecraft:custom_data.using_item.chest

execute if items entity @s armor.chest *[minecraft:custom_data~{using_item:{chest:{cooldown:{}}}}] run tag @s add ci.cd
execute if items entity @s armor.chest *[minecraft:custom_data~{using_item:{chest:{swap_items_on_trigger:{}}}}] run scoreboard players set %swap_item ci.math 1
execute store result score %consume ci.math if items entity @s armor.chest *[minecraft:custom_data~{using_item:{chest:{consume:{}}}}]
...
item modify entity @s armor.chest {function:"minecraft:copy_custom_data",source:{type:"minecraft:storage",source:"ci:item"},ops:[{source:"cooldown",target:"using_item.chest.cooldown",op:"merge"}]}
