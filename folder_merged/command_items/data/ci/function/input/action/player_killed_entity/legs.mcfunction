#REQUIRED ARGUMENTS equipment.legs player_killed_entity legs 

#CHECK IF SWAP
execute if items entity @s armor.legs *[minecraft:custom_data~{player_killed_entity:{legs:{only_command:{}}}}] run return run function ci:command with entity @s equipment.legs.components.minecraft:custom_data.player_killed_entity.legs

tag @s remove ci.cd
data remove storage ci:temp data
data modify storage ci:temp data set from entity @s equipment.legs.components.minecraft:custom_data.player_killed_entity.legs

execute if items entity @s armor.legs *[minecraft:custom_data~{player_killed_entity:{legs:{cooldown:{}}}}] run tag @s add ci.cd
execute if items entity @s armor.legs *[minecraft:custom_data~{player_killed_entity:{legs:{swap_items_on_trigger:{}}}}] run scoreboard players set %swap_item ci.math 1
execute store result score %consume ci.math if items entity @s armor.legs *[minecraft:custom_data~{player_killed_entity:{legs:{consume:{}}}}]


execute as @s[tag=!ci.cd] if score %consume ci.math matches 1.. run item modify entity @s armor.legs {function:"minecraft:set_count",count:-1,add:1}
...
execute if score %consume ci.math matches 1.. run item modify entity @s armor.legs {function:"minecraft:set_count",count:-1,add:1}
...
item modify entity @s armor.legs {function:"minecraft:copy_custom_data",source:{type:"minecraft:storage",source:"ci:item"},ops:[{source:"cooldown",target:"player_killed_entity.legs.cooldown",op:"merge"}]}
