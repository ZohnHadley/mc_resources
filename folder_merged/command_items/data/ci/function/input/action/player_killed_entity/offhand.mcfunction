#REQUIRED ARGUMENTS equipment.offhand player_killed_entity offhand 

#CHECK IF SWAP
execute if items entity @s weapon.offhand *[minecraft:custom_data~{player_killed_entity:{offhand:{only_command:{}}}}] run return run function ci:command with entity @s equipment.offhand.components.minecraft:custom_data.player_killed_entity.offhand

tag @s remove ci.cd
data remove storage ci:temp data
data modify storage ci:temp data set from entity @s equipment.offhand.components.minecraft:custom_data.player_killed_entity.offhand

execute if items entity @s weapon.offhand *[minecraft:custom_data~{player_killed_entity:{offhand:{cooldown:{}}}}] run tag @s add ci.cd
execute if items entity @s weapon.offhand *[minecraft:custom_data~{player_killed_entity:{offhand:{swap_items_on_trigger:{}}}}] run scoreboard players set %swap_item ci.math 1
execute store result score %consume ci.math if items entity @s weapon.offhand *[minecraft:custom_data~{player_killed_entity:{offhand:{consume:{}}}}]


execute as @s[tag=!ci.cd] if score %consume ci.math matches 1.. run item modify entity @s weapon.offhand {function:"minecraft:set_count",count:-1,add:1}
...
execute if score %consume ci.math matches 1.. run item modify entity @s weapon.offhand {function:"minecraft:set_count",count:-1,add:1}
...
item modify entity @s weapon.offhand {function:"minecraft:copy_custom_data",source:{type:"minecraft:storage",source:"ci:item"},ops:[{source:"cooldown",target:"player_killed_entity.offhand.cooldown",op:"merge"}]}
