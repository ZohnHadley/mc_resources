#REQUIRED ARGUMENTS equipment.feet player_killed_entity feet 

#CHECK IF SWAP
execute if items entity @s armor.feet *[minecraft:custom_data~{player_killed_entity:{feet:{only_command:{}}}}] run return run function ci:command with entity @s equipment.feet.components.minecraft:custom_data.player_killed_entity.feet

tag @s remove ci.cd
data remove storage ci:temp data
data modify storage ci:temp data set from entity @s equipment.feet.components.minecraft:custom_data.player_killed_entity.feet

execute if items entity @s armor.feet *[minecraft:custom_data~{player_killed_entity:{feet:{cooldown:{}}}}] run tag @s add ci.cd
execute if items entity @s armor.feet *[minecraft:custom_data~{player_killed_entity:{feet:{swap_items_on_trigger:{}}}}] run scoreboard players set %swap_item ci.math 1
execute store result score %consume ci.math if items entity @s armor.feet *[minecraft:custom_data~{player_killed_entity:{feet:{consume:{}}}}]
...
item modify entity @s armor.feet {function:"minecraft:copy_custom_data",source:{type:"minecraft:storage",source:"ci:item"},ops:[{source:"cooldown",target:"player_killed_entity.feet.cooldown",op:"merge"}]}
