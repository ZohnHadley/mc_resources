#REQUIRED ARGUMENTS equipment.legs use_item legs 

#CHECK IF SWAP
execute if items entity @s armor.legs *[minecraft:custom_data~{use_item:{legs:{only_command:{}}}}] run return run function ci:command with entity @s equipment.legs.components.minecraft:custom_data.use_item.legs

tag @s remove ci.cd
data remove storage ci:temp data
data modify storage ci:temp data set from entity @s equipment.legs.components.minecraft:custom_data.use_item.legs

execute if items entity @s armor.legs *[minecraft:custom_data~{use_item:{legs:{cooldown:{}}}}] run tag @s add ci.cd
execute if items entity @s armor.legs *[minecraft:custom_data~{use_item:{legs:{swap_items_on_trigger:{}}}}] run scoreboard players set %swap_item ci.math 1
execute store result score %consume ci.math if items entity @s armor.legs *[minecraft:custom_data~{use_item:{legs:{consume:{}}}}]
...
item modify entity @s armor.legs {function:"minecraft:copy_custom_data",source:{type:"minecraft:storage",source:"ci:item"},ops:[{source:"cooldown",target:"use_item.legs.cooldown",op:"merge"}]}
