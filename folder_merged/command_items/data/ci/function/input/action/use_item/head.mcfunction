#REQUIRED ARGUMENTS equipment.head use_item head 

#CHECK IF SWAP
execute if items entity @s armor.head *[minecraft:custom_data~{use_item:{head:{only_command:{}}}}] run return run function ci:command with entity @s equipment.head.components.minecraft:custom_data.use_item.head

tag @s remove ci.cd
data remove storage ci:temp data
data modify storage ci:temp data set from entity @s equipment.head.components.minecraft:custom_data.use_item.head

execute if items entity @s armor.head *[minecraft:custom_data~{use_item:{head:{cooldown:{}}}}] run tag @s add ci.cd
execute if items entity @s armor.head *[minecraft:custom_data~{use_item:{head:{swap_items_on_trigger:{}}}}] run scoreboard players set %swap_item ci.math 1
execute store result score %consume ci.math if items entity @s armor.head *[minecraft:custom_data~{use_item:{head:{consume:{}}}}]
...
item modify entity @s armor.head {function:"minecraft:copy_custom_data",source:{type:"minecraft:storage",source:"ci:item"},ops:[{source:"cooldown",target:"use_item.head.cooldown",op:"merge"}]}
