#REQUIRED ARGUMENTS SelectedItem player_hurt_entity mainhand 

#CHECK IF SWAP
execute if items entity @s weapon.mainhand *[minecraft:custom_data~{player_hurt_entity:{mainhand:{only_command:{}}}}] run return run function ci:command with entity @s SelectedItem.components.minecraft:custom_data.player_hurt_entity.mainhand

tag @s remove ci.cd
data remove storage ci:temp data
data modify storage ci:temp data set from entity @s SelectedItem.components.minecraft:custom_data.player_hurt_entity.mainhand

execute if items entity @s weapon.mainhand *[minecraft:custom_data~{player_hurt_entity:{mainhand:{cooldown:{}}}}] run tag @s add ci.cd
execute if items entity @s weapon.mainhand *[minecraft:custom_data~{player_hurt_entity:{mainhand:{swap_items_on_trigger:{}}}}] run scoreboard players set %swap_item ci.math 1
execute store result score %consume ci.math if items entity @s weapon.mainhand *[minecraft:custom_data~{player_hurt_entity:{mainhand:{consume:{}}}}]
...
item modify entity @s weapon.mainhand {function:"minecraft:copy_custom_data",source:{type:"minecraft:storage",source:"ci:item"},ops:[{source:"cooldown",target:"player_hurt_entity.mainhand.cooldown",op:"merge"}]}
