#REQUIRED ARGUMENTS equipment.head player_hurt_entity head 

#CHECK IF SWAP
execute if items entity @s armor.head *[minecraft:custom_data~{player_hurt_entity:{head:{only_command:{}}}}] run return run function ci:command with entity @s equipment.head.components.minecraft:custom_data.player_hurt_entity.head

tag @s remove ci.cd
data remove storage ci:temp data
data modify storage ci:temp data set from entity @s equipment.head.components.minecraft:custom_data.player_hurt_entity.head

execute if items entity @s armor.head *[minecraft:custom_data~{player_hurt_entity:{head:{cooldown:{}}}}] run tag @s add ci.cd
execute if items entity @s armor.head *[minecraft:custom_data~{player_hurt_entity:{head:{swap_items_on_trigger:{}}}}] run scoreboard players set %swap_item ci.math 1
execute store result score %consume ci.math if items entity @s armor.head *[minecraft:custom_data~{player_hurt_entity:{head:{consume:{}}}}]
...
item modify entity @s armor.head {function:"minecraft:copy_custom_data",source:{type:"minecraft:storage",source:"ci:item"},ops:[{source:"cooldown",target:"player_hurt_entity.head.cooldown",op:"merge"}]}
