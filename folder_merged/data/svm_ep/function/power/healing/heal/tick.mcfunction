...
execute as @s[scores={svm_ep.using_move=..95}] unless predicate svm_ep:holding_ability run scoreboard players reset @s svm_ep.using_move
...
execute as @s[scores={svm_ep.using_move=80..}] run return 0
...
scoreboard players add @s svm_ep.p.healing_ability_01_delay 40
