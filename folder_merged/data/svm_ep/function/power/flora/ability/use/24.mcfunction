...
execute as @s[type=player,advancements={svm_ep:flora_visited_biome/plains=false}] run return 0
...
scoreboard players operation %cooldown1 svm_ep.numbers = @s svm_ep.p.flora_ability_24_delay
function svm_ep:system/cooldown/calculate
execute if score %cooldown1 svm_ep.numbers matches 1.. run return run function svm_ep:system/message/on_cooldown


scoreboard players set @s svm_ep.p.flora_ability_24_delay 180
scoreboard players operation @s[type=player] svm_ep.p.flora_ability_24_delay += @s svm_ep.tick
scoreboard players operation @s[type=!player] svm_ep.p.flora_ability_24_delay += %WORLD svm_ep.tick
...
scoreboard players set @s svm_ep.using_move 860
