...
execute as @s[type=player,advancements={svm_ep:flora_visited_biome/moss=false}] run return 0
...
scoreboard players operation %cooldown1 svm_ep.numbers = @s svm_ep.p.flora_ability_30_delay
function svm_ep:system/cooldown/calculate
execute if score %cooldown1 svm_ep.numbers matches 1.. run return run function svm_ep:system/message/on_cooldown


scoreboard players set @s svm_ep.p.flora_ability_30_delay 120
scoreboard players operation @s[type=player] svm_ep.p.flora_ability_30_delay += @s svm_ep.tick
scoreboard players operation @s[type=!player] svm_ep.p.flora_ability_30_delay += %WORLD svm_ep.tick
