#REQUIRED ARGUMENTS $(slot_nbt) $(input) $(slot) 

#CHECK IF SWAP
$execute if items entity @s $(container) *[minecraft:custom_data~{$(input):{$(slot):{only_command:{}}}}] run return run function ci:command with entity @s $(slot_nbt).components.minecraft:custom_data.$(input).$(slot)

tag @s remove ci.cd
data remove storage ci:temp data
$data modify storage ci:temp data set from entity @s $(slot_nbt).components.minecraft:custom_data.$(input).$(slot)

$execute if items entity @s $(container) *[minecraft:custom_data~{$(input):{$(slot):{cooldown:{}}}}] run tag @s add ci.cd
$execute if items entity @s $(container) *[minecraft:custom_data~{$(input):{$(slot):{swap_items_on_trigger:{}}}}] run scoreboard players set %swap_item ci.math 1
$execute store result score %consume ci.math if items entity @s $(container) *[minecraft:custom_data~{$(input):{$(slot):{consume:{}}}}]


$execute as @s[tag=!ci.cd] if score %consume ci.math matches 1.. run item modify entity @s $(container) {function:"minecraft:set_count",count:-1,add:1}
...
$execute if score %consume ci.math matches 1.. run item modify entity @s $(container) {function:"minecraft:set_count",count:-1,add:1}

#STOP IF ITEM CLEARED
$execute unless items entity @s $(container) * run return 0
...
$item modify entity @s $(container) {function:"minecraft:copy_custom_data",source:{type:"minecraft:storage",source:"ci:item"},ops:[{source:"cooldown",target:"$(input).$(slot).cooldown",op:"merge"}]}
