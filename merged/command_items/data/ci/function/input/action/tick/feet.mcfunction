#REQUIRED ARGUMENTS equipment.feet tick feet 

#CHECK IF SWAP
execute if items entity @s armor.feet *[minecraft:custom_data~{tick:{feet:{only_command:{}}}}] run return run function ci:command with entity @s equipment.feet.components.minecraft:custom_data.tick.feet

tag @s remove ci.cd
data remove storage ci:temp data
data modify storage ci:temp data set from entity @s equipment.feet.components.minecraft:custom_data.tick.feet

execute if items entity @s armor.feet *[minecraft:custom_data~{tick:{feet:{cooldown:{}}}}] run tag @s add ci.cd
execute if items entity @s armor.feet *[minecraft:custom_data~{tick:{feet:{swap_items_on_trigger:{}}}}] run scoreboard players set %swap_item ci.math 1
execute store result score %consume ci.math if items entity @s armor.feet *[minecraft:custom_data~{tick:{feet:{consume:{}}}}]
...
item modify entity @s armor.feet {function:"minecraft:copy_custom_data",source:{type:"minecraft:storage",source:"ci:item"},ops:[{source:"cooldown",target:"tick.feet.cooldown",op:"merge"}]}
