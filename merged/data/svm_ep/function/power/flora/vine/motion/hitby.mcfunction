...
execute as @s[tag=svm_ep.flora_vine_hitbox] run return 0
execute unless score @s svm_ep.entity_id = @n[tag=executor] svm_ep.entity_id run effect give @s minecraft:slowness 2 4 true

execute as @s[gamemode=spectator] run return 0

#execute as @s[type=!player] run function svm_ep:power/flora/vine/motion/hitby/non_player
#execute as @s[type=player] run function svm_ep:power/flora/vine/motion/hitby/player
#kill @n[tag=executor]
