...
execute as @s[type=player,advancements={svm_ep:flora_visited_biome/birch=false}] run return 0
...
scoreboard players remove @s svm_ep.mana_after_cost 30


execute as @s[scores={svm_ep.mana_after_cost=..-1}] run return run function svm_ep:system/message/not_enough_mana

scoreboard players operation %cooldown1 svm_ep.numbers = @s svm_ep.p.flora_ability_31_delay
function svm_ep:system/cooldown/calculate
execute if score %cooldown1 svm_ep.numbers matches 1.. run return run function svm_ep:system/message/on_cooldown


scoreboard players set @s svm_ep.p.flora_ability_31_delay 140
scoreboard players operation @s[type=player] svm_ep.p.flora_ability_31_delay += @s svm_ep.tick
scoreboard players operation @s[type=!player] svm_ep.p.flora_ability_31_delay += %WORLD svm_ep.tick
