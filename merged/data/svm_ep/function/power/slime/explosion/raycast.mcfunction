...
execute unless entity @s[distance=..30] run tag @s add end_raycast
execute as @s[tag=end_raycast,tag=!teleport] run tellraw @s {"text":"Requires slime block or slime at cursor!","color":"red"}
execute at @s[tag=end_raycast,tag=!teleport] run playsound minecraft:entity.villager.no master @s ~ ~ ~ 1 0.8

execute as @s[tag=!end_raycast] positioned ^ ^ ^0.2 run function svm_ep:power/slime/explosion/raycast
tag @s remove end_raycast
execute as @s[tag=teleport] run function svm_ep:power/slime/explosion/summon0
