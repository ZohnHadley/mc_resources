scoreboard players operation %cooldown1 svm_ep.numbers = @s svm_ep.p.copy_ability_35_delay
function svm_ep:system/cooldown/calculate

execute unless entity @s[scores={svm_ep.mana=25..}] run return run function svm_ep:system/message/not_enough_mana
execute unless entity @s[scores={svm_ep.p.copy_ability_35_count=1..}] run return run function svm_ep:system/message/copy_no_uses
...
scoreboard players set @s svm_ep.p.copy_ability_35_delay 6000
scoreboard players operation @s[type=player] svm_ep.p.copy_ability_35_delay += @s svm_ep.tick
scoreboard players operation @s[type=!player] svm_ep.p.copy_ability_35_delay += %WORLD svm_ep.tick
